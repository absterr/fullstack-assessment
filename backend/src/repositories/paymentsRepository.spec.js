...
describe("paymentsRepository.findPaymentByIdempotencyKey", () => {
  it("returns payment when key exists", async () => {
    ...
    const result =
      await paymentsRepository.findPaymentByIdempotencyKey("key-2");
    expect(pool.query).toHaveBeenCalledWith(
      expect.stringContaining("WHERE idempotency_key"),
      ["key-2"],
    );
  ...
  it("returns null when not found", async () => {
    ...
    const result =
      await paymentsRepository.findPaymentByIdempotencyKey("missing");
...
describe("paymentsRepository.createWebhookEvent", () => {
  it("inserts webhook event and returns the row", async () => {
    ...
    expect(pool.query).toHaveBeenCalledWith(
      ...
      ["evt-1", 2, "payment_succeeded", { foo: "bar" }],
    );
